...
class ElpVideoCapture 
{
    public:
               ...
               ~ElpVideoCapture     ();
        void    captureFrame        (const char* outputFileName, char* mem_ptr, int devId, int send_flag, int sock, const char* map_file);
    private:
        int         m_fd;
        char       *m_buffer;
...
void ElpVideoCapture::captureFrame (const char* outputFileName, char* mem_ptr, int devId, int send_flag, int sock, const char* map_file) 
{
    if (false == m_isOpened) {
    ...
    memcpy(mem_ptr+devId*IMG_WIDTH*IMG_HEIGHT, image_ptr, IMG_WIDTH*IMG_HEIGHT);
    ...
    for (int y=0; y < IMG_HEIGHT; y++)   
    {
        ...
        memcpy(mem_ptr + devId*IMG_WIDTH*IMG_HEIGHT, image_ptr + ( v_offs + y )*image.cols + h_offs, IMG_WIDTH );
    ...
    if(send_flag)
    {
        send(sock , mem_ptr+devId*IMG_WIDTH*IMG_HEIGHT , IMG_HEIGHT*IMG_WIDTH , 0 ); 
...
int main (int argc, char* argv[]) 
{
    ...
    //testing on PC
    //char* mem_ptr = (char*) malloc(2*IMG_WIDTH*IMG_HEIGHT);
    ...
    *(reg_ptr + sgm1_block_number*0x04000 + 0) = *(reg_ptr + sgm1_block_number*0x04000 + 0) | 0b10000001; 
    ...
    //it is observed that video1 is the left camera and video0 is right which is oppoiste of our assumption
    ...
    //syntax: captureFrame(const char* outputFileName, char* mem_ptr, int devId, int send_flag, int sock);
    //capture frames
    for (int t = 0; t < NUM_IMAGES; t++) 
    {
        ...
        if (0 == t) 
        {
            for (int r = 0; r < THROW_AWAY_FRAMES; r++) 
            {
                ...
                {
                    devId ? v1.captureFrame (FRAME_1,mem_ptr,devId,0,sock_right,RIGHT_RMAP) : v0.captureFrame (FRAME_0,mem_ptr,devId,0,sock_left,LEFT_RMAP);  
                }            
            }
        }
        ...
        for(int devId=0; devId<2; devId++)
        {
            devId ? v1.captureFrame (FRAME_1,mem_ptr,devId,SEND_FLAG,sock_right,RIGHT_RMAP) : v0.captureFrame (FRAME_0,mem_ptr,devId,SEND_FLAG,sock_left,LEFT_RMAP);  
        }
